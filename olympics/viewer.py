...
class Viewer():
    ...
    def draw_direction(self, pos_list, a_list):
        ...
        for i in range(len(pos_list)):
            ...
            if a_x != 0 or a_y != 0:
                ...
                pygame.draw.line(self.background, color=[0, 0, 0], start_pos=[start_x, start_y], end_pos=[end_x, end_y],
                                 width=2)
    ...
    def draw_map(self, object):
        ...
        if object.type == 'arc':
            pygame.draw.arc(self.background, COLORS[object.color], object.init_pos, object.start_radian,
                            object.end_radian, object.width)
            ...
            s, e = object.init_pos
            pygame.draw.line(surface=self.background, color=COLORS[object.color], start_pos=s, end_pos=e,
                             width=object.width)
    ...
    def draw_view(self, obs, agent_list):  # obs: [2, 100, 100] list
        ...
        for agent_idx in range(len(obs)):
            ...
            for row in matrix:
                ...
                for item in row:
                    pygame.draw.rect(self.background, COLORS[IDX_TO_COLOR[int(item)]],
                                     [x, y, grid_node_width, grid_node_height])
                    x += grid_node_width
                y += grid_node_height

            pygame.draw.circle(self.background, COLORS[agent_list[agent_idx].color],
                               [coord[agent_idx] + 10, 55 + agent_list[agent_idx].r],
                               agent_list[agent_idx].r, width=0)
            pygame.draw.circle(self.background, COLORS["black"], [coord[agent_idx] + 10, 55 + agent_list[agent_idx].r],
                               2,
                               width=0)

            pygame.draw.lines(self.background, points=[[566 + 70 * agent_idx, 5], [566 + 70 * agent_idx, 55],
                                                       [566 + 50 + 70 * agent_idx, 55], [566 + 50 + 70 * agent_idx, 5]],
                              closed=True,
